% ----------------------------------------------
%
%	Damian Skrzypiec
% 	20.04.2017
%	Draft of Master Thesis - Mathematics
%
% ----------------------------------------------

\documentclass{pracamgr}



\usepackage[UTF8]{inputenc}
\usepackage{latexsym}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{amssymb}
\usepackage{algorithm}
\usepackage{algpseudocode}
\usepackage{listings}
\usepackage{hyperref}

% ----------------------
% For Geogebra
% ----------------------

\usepackage{pgf,tikz}
\usetikzlibrary{shapes.geometric}
\usepackage{color}
\usetikzlibrary{arrows}
\usepackage{xcolor}
%\usepackage[latin2]{inputenc}
%\usepackage[utf8]{inputenc}
\usepackage{latexsym}
\usepackage{graphicx,wrapfig,lipsum}

\definecolor{qqqqff}{rgb}{0.,0.,1.}
\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}

% Symbol for conditional independence
\newcommand*{\bigCI}{%
  \mathrel{\text{%
    {\rotatebox[origin=c]{90}{\resizebox{2.25ex}{1.65ex}{$\vDash$}}}%
  }}%
}

\theoremstyle{definition}

% Propositions 
\newtheorem{prop}{Proposition}[section]
\newtheorem{thm}{Theorem}[section]
\newtheorem{ex}{Example}[section]
\newtheorem{lemma}{Lemma}[section]
\newtheorem*{prf}{Proof}
\newtheorem{remark}{Remark}[section]


% Definition of symbol C-Separation
\newcommand{\cSep}[4]{\ensuremath{\left\langle #1 , #2 \mid #3 \right\rangle_{\mathcal{#4}}^{sep}}}

% Short for \mathcal{}
\newcommand{\mc}[1]{\ensuremath{\mathcal{#1}}}
\newcommand{\mb}[1]{\ensuremath{\mathbb{#1}}}
\newcommand*{\QED}{\hfill\ensuremath{\blacksquare}}

% Dane magistranta:

\author{Damian Skrzypiec}
\nralbumu{320335}
\title{Structure Learning Algorithms for Chain Graphs}

\tytulang{Algorytmy uczenia strukturalnego dla grafów łańcuchowych}
\kierunek{Mathematics}
\opiekun{John Noble, PhD.\\
  Institute of Applied Mathematics and Mechanics\\
  }

\date{December 2017}

\dziedzina{ 
11.2 Statistics\\ 
}

%Klasyfikacja tematyczna wedlug AMS (matematyka) lub ACM (informatyka)
\klasyfikacja{62 Statistics \\
		62-09 Graphical methods \\
		62E10 Characterization and structure theory
		}

% Słowa kluczowe:
\keywords{graphical model, chain graph, structure learning}

% Tu jest dobre miejsce na Twoje własne makra i~środowiska:
\newtheorem{defi}{Definition}[section]

% koniec definicji

\begin{document}
\maketitle

\begin{abstract}
	In this paper we present an algorithm for structural learning of chain graphs. Chain graphs 
	are class of probabilistic graphical models which can be described as generalization of Bayesian networks and 
	Markov random fields. We discuss in details LCD algorithm which is an algorithm for structural learning of
	chain graphs based on idea of decomposition chain graphs via separation trees. We also provide implementation
	of LCD algorithm available via R package \textit{cglearn}. 
\end{abstract}

\tableofcontents
\listoffigures
\listofalgorithms
\addtocontents{loa}{\def\string\figurename{Algorithm}}
%\listoftables

\chapter{Introduction}

\input{Sections/Introduction}


\chapter{Preliminaries}\label{r:prelim}

	\section{Graph Theory Terminology}\label{r:defGraph}
		\input{Sections/Preliminaries/GraphDefinitions}
		
	\section{Graphical Model Terminology}\label{r:defGraphModel}
		\input{Sections/Preliminaries/GraphicalModelDefinitions}


\chapter{LCD Algorithm} \label{chap:LCDAlg}

	\input{Sections/LCDAlgorithm/Overview}

	\section{Decomposition of chain graphs}
		\input{Sections/LCDAlgorithm/SeparationTreesOverview}
		
		\subsection{Separation Trees} \label{subsec:SepTrees}
			\input{Sections/LCDAlgorithm/SeparationTrees}
			
		\subsection{Construction of Separation Trees} \label{subsec:SepTreesConstruct}
			\input{Sections/LCDAlgorithm/ConstructionOfSeparationTrees}			

		\subsection{Separation Tree Lemmas}			
			\input{Sections/LCDAlgorithm/SeparationTreeLemma}
		
	\section{Algorithm}
		\input{Sections/LCDAlgorithm/AlgOverview}
			
		\subsection{Mathematical basis} \label{subsec:LCDMathBasis}
			\input{Sections/LCDAlgorithm/MathematicalBasis}	
	
		\subsection{Skeleton recovery} \label{subsec:LCDSkeletonRecovery}
			\input{Sections/LCDAlgorithm/SkeletonRecovery}	
	
		\subsection{Complex recovery} \label{subsec:LCDComplexRecovery}
			\input{Sections/LCDAlgorithm/ComplexRecovery}
	
		\subsection{Algorithm complexity} \label{subsec:LCDAlgComplex}
			\input{Sections/LCDAlgorithm/Complexity}
			
\chapter{Implementation}
	\input{Sections/Implementation/ImplementOverview}

	\section{Package Functionality} \label{subsec:PackageFunc}
		\input{Sections/Implementation/PackageFunc}

	\section{Package Installation} \label{subsec:PackageInstall}
		\input{Sections/Implementation/PackageInstall}
		
	% \section{Technicals} - optional
	
\chapter{Results}
	\input{Sections/Results/Result.tex}

\chapter{Summary}
	\input{Sections/Summary/Summary.tex}


%\chapter{CKES Algorithm}
%
%	\input{Sections/CKESAlgorithm/Overview}
%
%	\section{Mathematical basis} \label{sec:CKESMathBasis}
%		\input{Sections/CKESAlgorithm/MathematicalBasis}
%
%	\section{Algorithm} \label{sec:CKESAlg}
%		\input{Sections/CKESAlgorithm/Algorithm}
%		
%		\subsection{Algorithm complexity}

\begin{thebibliography}{99}
	\addcontentsline{toc}{chapter}{Bibliography}
	\bibitem{CGMP} M. Frydenberg, \textit{The Chain Graph Markov Property}, Scandinavian Journal of Statistics 17 (1990) 333-353	
	
	\bibitem{CG} Z. Ma, X. Xie and Z. Geng, \textit{Structural Learning Of Chain Graphs via Decomposition}, Journal of Machine Learning Research 9 (2008) 2847-2880

	\bibitem{CKES} J. Nielsen, J. Pena and D. Sonntag, \textit{An inclusion optimal algorithm for chain graph structure learning}, Proceedings of the Seventeenth International
		Conference on Artificial Intelligence and Statistics, PMLR (2014) 33:778-786

	\bibitem{FaithDicr} J. Pena, \textit{Faithfulness in Chain Graphs: The Discrete Case}, International Journal of Approximate Reasoning 50 (2009) 1306-1313

	\bibitem{FaithGauss} J. Pena, \textit{Faithfulness in Cahin Graphs: The Gaussian Case}, Proceedings of the Fourteenth International Conference on Artificial Intelligence and Statistics (2011) 588-599

	\bibitem{LBO} A. Roverto and L. La Rocca, \textit{On Block Ordering of Variables in Graphical Modelling}, Scandinavian Journal of Statistics 33 (2006) 65-81

	\bibitem{OCG} M. Studeny and R.R. Bouckaert, \textit{On chain graph models for description of conditional independence structures}, Annals Of Statistics 26 (1998) 1434-1495
	
	\bibitem{GIT} GitHub repository containing source code of \textit{cglearn} package, \url{https://github.com/DSkrzypiec/cglearn} (03.12.2017)
\end{thebibliography}

\end{document}


%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% coding: latin-2
%%% End:
